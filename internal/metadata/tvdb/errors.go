...
// Error types for TVDB client operations.
...
func (e *ClientError) Error() string {
	if e.RetryAfter > 0 {
		return fmt.Sprintf("tvdb: %s (HTTP %d, retry after %ds): %s", e.Code, e.StatusCode, e.RetryAfter, e.Message)
	}
	return fmt.Sprintf("tvdb: %s (HTTP %d): %s", e.Code, e.StatusCode, e.Message)
}
...
// NewUnauthorizedError creates a 401 error (invalid credentials).
