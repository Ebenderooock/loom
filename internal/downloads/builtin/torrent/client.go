...
type Client struct {
	...
	lastSummary     downloads.TorrentEngineSummary
	summaryUpdateAt time.Time
}
...
func (c *Client) torrentToItem(item *torrentItem) downloads.Item {
	...
	// Calculate upload ratio from stats
	stats := t.Stats()
	var ratio float64
	if t.BytesCompleted() > 0 {
		ratio = float64(stats.BytesReadData.Int64()) / float64(t.BytesCompleted())
	}
	...
	return downloads.Item{
		...
		Ratio:           ratio,
		SavePath:        item.SavePath,
		ContentPath:     t.Name(), // torrent's main file/dir name
	}
}
...
// Recheck implements DownloadClient.Recheck (stub for MVP).
func (c *Client) Recheck(ctx context.Context, ids ...string) error {
	return fmt.Errorf("torrent: Recheck not yet implemented")
}

// Reannounce implements DownloadClient.Reannounce (stub for MVP).
func (c *Client) Reannounce(ctx context.Context, ids ...string) error {
	return fmt.Errorf("torrent: Reannounce not yet implemented")
}
...
// FreeSpace implements DownloadClient.FreeSpace.
...
func (c *Client) EngineSummary() downloads.TorrentEngineSummary {
	...
	return downloads.TorrentEngineSummary{
		...
		Seeding:       seeding,
		Paused:        paused,
		DownloadRate:  0, // TODO: aggregate from stats
		UploadRate:    0,
...
func (c *Client) SetSpeedLimits(downBytesPerSec, upBytesPerSec int64) {
	...
	// TODO: apply to client via anacrolix/torrent
