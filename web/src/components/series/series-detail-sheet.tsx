...
const EPISODE_STATUS_STYLE: Record<
  EpisodeStatus,
...
function getSeasonStats(episodes: Episode[]) {
  ...
  const percent =
    aired > 0
      ? Math.round((downloaded / aired) * 100)
      : episodes.length > 0
        ? 100
        : 0;
  return {
...
function SeasonAccordion({
  ...
  const fetchEpisodes = async () => {
    try {
      const res = await apiFetch(
        `/api/v1/series/${seriesId}/seasons/${season.seasonNumber}/episodes`,
      );
      if (res.ok) {
  ...
  const handleAutoSearchEpisode = async (ep: Episode) => {
    ...
    const toastId = toast.loading(
      `Searching for ${seriesTitle} S${String(season.seasonNumber).padStart(2, "0")}E${String(ep.episodeNumber).padStart(2, "0")}...`,
    );
    try {
  ...
  const stats =
    ...
    (season.episodeStats
      ? {
          ...
          unaired:
            season.episodeStats.totalEpisodes -
            season.episodeStats.airedEpisodes,
          unmonitored:
            season.episodeStats.totalEpisodes -
            season.episodeStats.monitoredEpisodes,
          aired: season.episodeStats.airedEpisodes,
          total: season.episodeStats.totalEpisodes,
          percent:
            season.episodeStats.airedEpisodes > 0
              ? Math.round(
                  (season.episodeStats.downloadedEpisodes /
                    season.episodeStats.airedEpisodes) *
                    100,
                )
              : season.episodeStats.totalEpisodes > 0
                ? 100
                : 0,
        }
      : null);

  const badgeColor = !stats
    ? "bg-zinc-600"
    : stats.percent === 100
      ? "bg-green-600"
      : stats.percent > 0
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex w-16 shrink-0 items-center justify-end gap-0.5">
                          ...
                          >
                            {autoSearchingEp === ep.episodeNumber ? (
                              <Loader2 className="h-3.5 w-3.5 animate-spin" />
                            ) : (
                              <Search className="h-3.5 w-3.5" />
                            )}
...
export function SeriesDetailSheet({
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      const refreshRes = await apiFetch(`/api/v1/series/${series.id}/refresh`, {
        method: "POST",
      });
      if (!refreshRes.ok) {
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        >
          <SheetHeader className="sr-only">
            ...
            <SheetDescription>
              Series details for {series.title}
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="relative h-[300px] overflow-hidden bg-muted">
            {series.backdropPath ? (
              ...
              <img
                src={`${TMDB_IMG}/w780${series.posterPath}`}
                alt=""
          ...
          <div className="px-6 pb-2">
            <div className="flex items-center gap-1.5">
              ...
              <Button
                ...
                title="Automatic search for all episodes"
            ...
            <div className="mx-6 mb-3 space-y-4 rounded-lg border border-accent/30 bg-accent/5 p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label
                    htmlFor="series-edit-profile"
                    className="text-xs font-medium"
                  >
                    Quality Profile
                  </label>
                  <Select value={editProfile} onValueChange={setEditProfile}>
                    <SelectTrigger id="series-edit-profile">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {profiles.map((p) => (
                ...
                <div className="space-y-1.5">
                  <label
                    htmlFor="series-edit-monitoring"
                    className="text-xs font-medium"
                  >
                    Monitoring
                  </label>
                  ...
                  >
                    <SelectTrigger id="series-edit-monitoring">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={setEditSeriesType}
                  >
                    <SelectTrigger id="series-edit-type">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
          ...
          <div className="space-y-1 px-6 pb-6">
            ...
            <CollapsibleSection title="Overview" icon={Tv}>
              <div className="mb-4 grid grid-cols-2 gap-4">
                ...
                {profile && (
                  <DetailRow label="Quality">{profile.name}</DetailRow>
                )}
                {library && (
                  <DetailRow label="Library">{library.name}</DetailRow>
                )}
              </div>
                ...
                <div className="space-y-4">
                      ...
                      <div className="flex gap-3 overflow-x-auto pb-2">
                        {credits.cast.slice(0, 20).map((p) => (
                          <PersonCard
                            ...
                            person={p}
                            onClick={() =>
                              setDiscoverPerson({ id: p.id, name: p.name })
                            }
                          />
                        ))}
                      </div>
                  ...
                  {credits.crew?.length > 0 && (
                    <div>
                      ...
                      <div className="flex gap-3 overflow-x-auto pb-2">
                        {credits.crew.slice(0, 10).map((p) => (
                          <PersonCard
                            key={`${p.id}-${p.department}`}
                            person={p}
                            onClick={() =>
                              setDiscoverPerson({ id: p.id, name: p.name })
                            }
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete &ldquo;{series.title}&rdquo;? This
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
