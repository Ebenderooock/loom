...
test.describe("Movies Page", () => {
  ...
  test("movies page loads when authenticated", async ({ page }) => {
    ...
    await expect(page.locator("header").getByText("Movies")).toBeAttached({
      timeout: 10000,
    });
  });
  ...
  test("empty state shows import button when libraries exist", async ({
    page,
  }) => {
    // Override with empty movie list
    await page.route("**/api/v1/movies*", async (route) => {
  ...
  test("empty state shows library warning when no libraries", async ({
    page,
  }) => {
    ...
    await expect(page.getByText(/Add a library in Settings/i)).toBeVisible({
      timeout: 10000,
    });
  });
});
