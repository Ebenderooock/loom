...
test.describe("Workflows Page", () => {
  ...
  test("renders workflows page heading", async ({ page }) => {
    ...
    await expect(
      page.locator("header").getByText("Workflows"),
  ...
  test("shows all workflows in the table", async ({ page }) => {
    ...
    await expect(page.getByText("Test.Movie.2024.1080p.BluRay.x264")).toBeVisible({
      timeout: 10000,
    });
  ...
  test("filter tabs show correct counts", async ({ page }) => {
    ...
    // All tab shows total
    await expect(page.getByRole("tab", { name: /All \(3\)/ })).toBeVisible({ timeout: 10000 });
  ...
  test("active filter shows only active workflows", async ({ page }) => {
    ...
    await expect(page.getByText("Another.Movie.2023.720p.WEB-DL")).not.toBeVisible();
  ...
  test("completed filter shows only completed workflows", async ({ page }) => {
    ...
    await expect(page.getByText("Another.Movie.2023.720p.WEB-DL")).toBeVisible({ timeout: 10000 });
    await expect(page.getByText("Test.Movie.2024.1080p.BluRay.x264")).not.toBeVisible();
  });
  ...
  test("failed filter shows only failed workflows", async ({ page }) => {
    ...
    await expect(page.getByText("Test.Show.S01E05.HDTV")).toBeVisible({ timeout: 10000 });
    await expect(page.getByText("Test.Movie.2024.1080p.BluRay.x264")).not.toBeVisible();
  });
  ...
  test("displays state badges correctly", async ({ page }) => {
    ...
    await expect(table.getByText("Downloading")).toBeVisible({ timeout: 10000 });
  ...
  test("shows error message for failed workflow", async ({ page }) => {
    ...
    await expect(
      page.getByText(/all retries exhausted/),
  ...
  test("action menu opens for active workflow", async ({ page }) => {
    ...
    const row = page.locator("tr").filter({ hasText: "Test.Movie.2024.1080p.BluRay.x264" });
    await row.locator("button").last().click();

    // Cancel option should be visible for active workflows
    await expect(page.getByRole("menuitem", { name: /Cancel/ })).toBeVisible({ timeout: 5000 });
  });

  test("action menu opens for failed workflow with retry option", async ({ page }) => {
    await page.goto("/workflows");
    ...
    await expect(page.getByRole("menuitem", { name: /Retry/ })).toBeVisible({ timeout: 5000 });
  ...
  test("shows empty state when no workflows exist", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Workflows will appear here/),
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test("shows workflow type labels", async ({ page }) => {
    ...
    await expect(page.getByText("Movie Search").first()).toBeVisible({ timeout: 10000 });
