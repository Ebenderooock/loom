...
test.describe("Live: Events & Audit Log", () => {
  ...
  test("events page loads and shows audit entries", async ({ page }) => {
    ...
    await expect(page.getByText(/Events|Activity/i).first()).toBeVisible({ timeout: 15000 });
  });
  ...
  test("audit log API returns entries", async ({ request }) => {
    ...
    const entries = body.data ?? body.entries ?? (Array.isArray(body) ? body : []);
  ...
  test("audit log includes movie status changed events", async ({ request }) => {
    ...
    console.log("Found " + statusEvents.length + " movie.status_changed audit events");
  ...
  test("events SSE stream endpoint is reachable", async ({ request }) => {
    ...
    try {
      const loginRes = await request.post("/api/v1/auth/login", {
        data: { username: process.env.LOOM_USER || "admin", password: process.env.LOOM_PASS || "" },
      });
      const cookies = loginRes.headers()["set-cookie"] || "";
      const match = cookies.match(/loom_session=([^;]+)/);
      if (match) headers.Cookie = "loom_session=" + match[1];
    } catch (_) { /* proceed unauthenticated */ }
