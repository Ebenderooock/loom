...
test.describe("Live: Movies", () => {
  ...
  test("movies page loads and displays movies", async ({ page }) => {
    ...
    const emptyState = page
      .getByText(/No movies|Add your first|Add Movie/i)
      .first();
  ...
  test("movie detail opens on card click", async ({ page }) => {
    ...
    const detailContent = page
      .locator(
        "[role=dialog], [data-state=open], main h1, main h2, main [class*=detail]",
      )
      .first();
  ...
  test("movie statuses are valid values", async ({ request }) => {
    ...
    const validStatuses = [
      "missing",
      "available_right_quality",
      "available_wrong_quality",
      "downloading",
      "grabbed",
      "announced",
      "tba",
